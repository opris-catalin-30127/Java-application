...
public class DoorLockController implements ControllerInterface {
    ...
    ReadsSerializedFiles readsSerializedFiles=new ReadsSerializedFiles();

    private int attempts;

    private String fileName = "AccessLogs/accessLog-" + valueOf(LocalDateTime.now()).getTime() + ".dat";
    ...
    public DoorStatus enterPin(String pin) throws InvalidPinException, TooManyAttemptsException {
        ...
        if (!Objects.equals(tenant.getName(), MASTER_TENANT_NAME)) {
            if (!doorStatus) {
                attempts++;
                if (attempts >= 3) {
                    accessLog = new AccessLog(tenant.getName(), LocalDateTime.now(), "Enter pin", door.getStatus(), "Operation finished with TooManyAttemptsException ");
                    throw new TooManyAttemptsException("Too many attempts to entered pin");
                } else {
                    accessLog = new AccessLog(tenant.getName(), LocalDateTime.now(), "Enter pin", door.getStatus(), "Operation finished with InvalidPinException");
            ...
            accessLog = new AccessLog(tenant.getName(), LocalDateTime.now(), "Enter pin", door.getStatus(), "Successful operation");
        ...
        readsSerializedFiles.deserialize(fileName);
        readsSerializedFiles.display();
    ...
    public void addTenant(String pin, String name) throws TenantAlreadyExistsException {
        ...
        if (!validAccess.containsKey(tenant)) {
            ...
            accessLog = new AccessLog(name, LocalDateTime.now(), "Add tenant", door.getStatus(), "successful operation ");
        } else {
            accessLog = new AccessLog(name, LocalDateTime.now(), "Add tenant", door.getStatus(), "Operation finished with TenantAlreadyExistsException");
    ...
    public void removeTenant(String name) throws TenantNotFoundException {
        ...
        if (this.validAccess.containsKey(tenant)) {
            ...
            accessLog = new AccessLog(name, LocalDateTime.now(), "Remove Tenant", door.getStatus(), "successful operation");
            System.out.println("The tenant with name: " + name + " was removed");
        } else {
            accessLog = new AccessLog(name, LocalDateTime.now(), "Remove tenant", door.getStatus(), "Operation finished with TenantNotFoundException");
